 ...
 *  Popis: Obsahuje implementaci syntaktického analyzátoru
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int bottom_up(t_table precedence_table, t_node *return_node, tToken *token, tToken *token_backup, char *backup, bool bracket)
{
    ...
    while(1)
    {
        ...
        //if first token f expr is operator ->
        if(first && strcmp(item, "id") && strcmp(item, "expr") && strcmp(item, "(") && strcmp(item, "#"))
        {
            return_code = 2;
            continue;
        }
    ...
    if(strcmp(stack_top(stack), "expr") && strcmp(stack_top(stack), "id"))
        return_code = 2;
