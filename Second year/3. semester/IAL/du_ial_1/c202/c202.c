...
void Stack_Init( Stack *stack ) {
	...
	//pokud je předán parametr stack == NULL, vypíšeme chybovou zprávu
	if(stack == NULL){
		Stack_Error(SERR_INIT);
		return;
	}
	//nastavíme vrchol na -1, protože prázdný stack má hodnotu -1
	stack->topIndex = -1;
}
...
int Stack_IsEmpty( const Stack *stack ) {
	...
	//pokud je stack prázdný, index je nastaven na -1
...
int Stack_IsFull( const Stack *stack ) {
	...
	//pokud je stack plný, index je nastaven na velikost-1
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	...
	//ošetříme stav, kdy můžeme brát z vrchou prázdného stacku 
	if(Stack_IsEmpty(stack)){
		Stack_Error(SERR_TOP);
		return;
	}
	//jinak předáme hodnotu arraye na indexu topIndex
...
void Stack_Pop( Stack *stack ) {
	...
	//při prázdném stacku nic nedělej
	if (Stack_IsEmpty(stack)){
		return;
	}
	//jinak stačí snížit topIndex
	--stack->topIndex;
}
...
void Stack_Push( Stack *stack, char data ) {
	...
	//ošetříme situaci, kdy můžeme předávat do plného stacku
	if (Stack_IsFull(stack)){
		Stack_Error(SERR_PUSH);
		return;
	}
	//jinak zvýšíme topIndex předáme data do arraye na pozici topIndexu
	++stack->topIndex;
